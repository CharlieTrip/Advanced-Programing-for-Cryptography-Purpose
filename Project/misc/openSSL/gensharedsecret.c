...
int main(int argc, char* argv[]) {
    // arg1 should be mod
    // arg2 should be expo
    // arg3 should be the other public value
    BIGNUM *mod, *expo, *otherPubVal;
    mod = BN_new();
    expo = BN_new();
    otherPubVal = BN_new();
    ...
    BN_dec2bn(&mod,argv[1]);
    BN_dec2bn(&expo,argv[2]);
    BN_dec2bn(&otherPubVal,argv[3]);

    BIGNUM *secret;
    secret = BN_new();
    secret = modExpo(otherPubVal,expo,mod,ctx);
    printf("secret is: %s\n",BN_bn2dec(secret));
