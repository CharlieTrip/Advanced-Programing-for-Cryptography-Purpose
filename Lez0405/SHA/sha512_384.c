...
//starting constants
const uint32_t k512[64]={0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
...
void sha512_core(uint32_t *message,unsigned long long lenght,unsigned char *res,uint32_t H[], const uint32_t k[]);

uint32_t *sha512_padding(unsigned char *word,uint64_t word_lenght8,unsigned long long *lenght_32);

void sha512(unsigned char *word,long int word_lenght,unsigned char *res){
    ...
    uint32_t *padded_message = sha512_padding(to_hash,word_lenght,&lenght); //do not need to free to_hash
    
    uint32_t H[]={0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4};
    
    sha512_core(padded_message, lenght,res,H,k512);
    
    free(padded_message);
}

void sha512_core(uint32_t *message,unsigned long long lenght,unsigned char *res,
...
uint32_t *sha512_padding(unsigned char *word,uint64_t word_lenght8,unsigned long long *lenght_32){
    ...
    //TODO: need 128 bit int
    uint64_t n_zeros8=(1025-(word_lenght8*8-897)%1024)/8;
    //output lenght
